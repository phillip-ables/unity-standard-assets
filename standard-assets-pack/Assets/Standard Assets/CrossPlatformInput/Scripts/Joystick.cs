...
namespace UnityStandardAssets.CrossPlatformInput
{
	public class Joystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
	{
		...
		void CreateVirtualAxes()  // this is called first, this is basically called on start but when its enabled
		{
			...
			{
				m_HorizontalVirtualAxis = new CrossPlatformInputManager.VirtualAxis(horizontalAxisName);  // dont fully get this, not even a little
				CrossPlatformInputManager.RegisterVirtualAxis(m_HorizontalVirtualAxis);  // aside from the reference this is the entire four lines of CrossPlatformInputManager
			}
			if (m_UseY)
			{
